...
from PyQt5.QtCore import Qt, QUrl, QPoint, QTimer, QRect, QByteArray
...
class LineNumberArea(QFrame):
    def __init__(self, editor):
        ...
        self.editor = editor
        # Width is now set dynamically in CustomTextEdit
        self.marked_line = -1
...
class SyntaxHighlighter:
    ...
    def load_highlighting_rules(self):
        ...
        if not self.ide.settings.get("last_folder"):
            self.ide.settings["last_folder"] = os.path.expanduser("~")  # Initialize last_folder
        if not os.path.exists(config_dir):
            os.makedirs(config_dir)
        
        # Check if language file exists in user directory
        if not os.path.exists(language_file):
            ...
            if os.path.exists(fallback_path):
                try:
                    ...
                    return  # Fail if copy unsuccessful
            else:
                if SHOW_HL_INFO:
...
class CustomTextEdit(QTextEdit):
    ...
    def line_number_area_width(self):
        if not self.ide.settings["line_numbers"]:
            return 0
        # Calculate width based on the number of lines
        doc = self.document()
        line_count = doc.blockCount()
        digits = len(str(max(1, line_count)))  # Number of digits in the largest line number
        font = QFont("Consolas", self.ide.settings["editor_font_size"])
        fm = QFontMetrics(font)
        # Base width for up to 3 digits (999 lines) is 40 pixels
        # Add 8 pixels per additional digit beyond 3
        base_width = 40
        extra_digits = max(0, digits - 3)
        width = base_width + (extra_digits * 8)
        # Ensure minimum width by checking the actual text width
        max_number_width = fm.width(str(line_count) + " ")
        return max(width, max_number_width + 10)  # Add padding for aesthetics
...
class IDE(QMainWindow):
    def __init__(self, filename=None):
        super().__init__()
        self.setWindowTitle("GCBASIC Essential IDE 1.6")
        config_dir = os.path.expanduser("~/.superide")
        # Initialize recent files path
        ...
        self.settings = {
            ...
            "recent_files_path": self.recent_files_path  # Store for reference
        }
        self.recent_files = []  # List of {"name": filename, "path": full_path}
        self.file_cache = {}
        self.history = {}
        self.file_states = {}
        self.file_menu = None
        ...
        self.task_output_cache = []  # Cache for task output lines
        self.terminal = TerminalWindow()
        self.dock = QDockWidget("Terminal", self)
        self.dock.setObjectName("TerminalDock")  # Set unique objectName for saveState
        ...
        # Initialize timer for checking file changes
    ...
    def on_tab_changed(self, index):
        """Check the newly active tab for external changes."""
        if index >= 0:
    ...
    def init_ui(self):
        ...
        self.addAction(self.open_tasks_action)  # Add to main window for global scope
        ...
        about_action.triggered.connect(lambda: QMessageBox.information(self, "About", "GCBASIC Essential IDE v1.6"))
    ...
    def populate_tasks_menu(self):
        """Populate the IDE Tasks menu with tasks from tasks.json and assign shortcuts."""
        ...
        if not tasks:
            ...
            for task in tasks:
                ...
                if isinstance(label, str):
                    # Remove [..] and all contents between
    ...
    def open_tasks_menu(self):
        """Programmatically open the IDE Tasks menu."""
        if not self.ide_tasks_menu:
            self.terminal.log("IDE Tasks menu not initialized", "ERROR")
            return
        try:
            # Repopulate menu to ensure latest tasks
            self.populate_tasks_menu()
            # Get the position of the IDE Tasks menu in the menu bar
    ...
    def parse_tasks_json(self, file_path):
        """Parse tasks.json, handling comments and extracting function key shortcuts."""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            # Remove comment lines (assuming // or # style comments)
            cleaned_lines = []
            for line in lines:
                line = line.strip()
            ...
            # Process each task for shortcut
            for task in tasks:
                label = task.get("label", "Unnamed Task")
                # Check for [F1-12] or [ShiftF1-12], [Shift F1-12], [Shift+F1-12], [Shift +F1-12] (case-insensitive)
                match = re.search(r'\[(Shift\s*\+?\s*)?F([1-9]|1[0-2])\]', label, re.IGNORECASE)
                if match:
                    ...
                    if SHOW_HL_INFO:
                        self.terminal.log(f"Parsed shortcut '{shortcut}' for task '{label}'", "INFO")
                else:
                    task["shortcut"] = None
                    if SHOW_HL_INFO:
    ...
    def get_tasks_file_path(self):
        """Determine the tasks.json file path, copying from local folder to user directory if needed."""
    ...
    def run_task(self, task=None):
        """Execute a specific task or show dialog to select one."""
        # For GCBASIC.EXE tasks without "debug" in label, require an open tab
        if task and task.get("command", "").lower().__contains__("gcbasic.exe") and not "debug" in task.get("label", "").lower():
            current_tab = self.tabs.currentWidget()
            if not current_tab or not hasattr(current_tab, "file_path"):
                self.terminal.log("No file open to run GCBASIC task", "ERROR")
                return
            self.execute_task(task, current_tab.file_path)
        else:
            # For other tasks, use empty file_path if no tab
            current_tab = self.tabs.currentWidget()
            file_path = current_tab.file_path if current_tab and hasattr(current_tab, "file_path") else ""
            if not task:
                tasks_file = self.get_tasks_file_path()
                if not tasks_file:
                    self.terminal.log("No tasks file found", "ERROR")
                    return
                tasks = self.parse_tasks_json(tasks_file)
                if not tasks:
                    self.terminal.log("No tasks found in tasks.json", "ERROR")
                    return
                # Create a dialog to select a task
                dialog = QDialog(self)
                dialog.setWindowTitle("Select Task")
                layout = QVBoxLayout()
                combo = QComboBox()
                task_labels = [t.get("label", "Unnamed Task") for t in tasks]
                combo.addItems(task_labels)
                ...
                if dialog.exec_() == QDialog.Accepted:
                    selected_task = tasks[combo.currentIndex()]
                    self.execute_task(selected_task, file_path)
            else:
                self.execute_task(task, file_path)

    def execute_task(self, task, file_path=""):
        """Execute the selected task, handling browser launch for explorer, file deletion for Remove-Item, or using the operating system shell."""
        try:
            ...
            # Initialize defaults
            dirname = os.getcwd()  # Default to current working directory if no tab
            local_file_path = file_path  # Use parameter, reassign for GCBASIC tasks
            # Handle GCBASIC.EXE tasks without "debug" in label (require open tab)
            ...
            # Handle explorer command (launch default web browser)
    ...
    def open_language_file(self):
        """Open the language JSON file in the editor."""
    ...
    def open_tasks_file(self):
        """Open the tasks JSON file in the editor."""
        tasks_file = self.settings.get("tasks_file")
        if os.path.exists(tasks_file):
    ...
    def save_recent_files(self):
        """Save recent files to GCstudio.mrf.json, padding to 10 entries with empty strings."""
        try:
            # Create lists with exactly 10 entries, padding with ""
    ...
    def open_file(self):
        ...
        if not os.path.exists(last_folder):
            last_folder = os.path.expanduser('~')  # Fallback to home directory
    ...
    def open_file_by_path(self, file_path):
        ...
        self.save_recent_files()
        # Store the last accessed folder
            ...
            if current_tab.file_path.startswith("untitled_"):
                ...
                try:
                    ...
                    if current_tab.file_path in self.file_cache:
                        del self.file_cache[current_tab.file_path]  # Clear cache to prevent stale data
                    current_mtime = os.path.getmtime(current_tab.file_path)
                    self.file_states[current_tab.file_path] = (current_mtime, None)  # Update file state
            ...
            if file_path:
                try:
                    ...
                    if file_path in self.file_cache:
                        del self.file_cache[file_path]  # Clear cache to prevent stale data
                    current_mtime = os.path.getmtime(file_path)
                    self.file_states[file_path] = (current_mtime, None)  # Update file state
    ...
    def reset_terminal_position(self):
        """Reset the Terminal to its default docked position (bottom, 30% height)."""
    ...
    def apply_terminal_settings(self):
        """Apply Terminal visibility and size settings, respecting dock position."""
    ...
    def load_settings(self):
        ...
        try:
            with open(settings_path, "r", encoding="utf-8") as f:
                ...
                # Load recent files from GCstudio.mrf.json
                try:
                    if os.path.exists(self.recent_files_path):
                        ...
                        # Create GCstudio.mrf.json if it doesn't exist
                        self.recent_files = []
                        self.save_recent_files()
                ...
                if "dock_state" in loaded_settings:
                    try:
                        ...
                        self.addDockWidget(Qt.BottomDockWidgetArea, self.dock)  # Fallback to default
                else:
                    self.addDockWidget(Qt.BottomDockWidgetArea, self.dock)  # Default position
                self.apply_screen_size_and_position()
                self.apply_text_settings()
                self.apply_theme()
                self.apply_terminal_settings()
            ...
            try:
                ...
                self.terminal.log(f"Error setting defaults: {str(e)}", "ERROR")
            ...
            self.settings["editor_font_size"] = 12
            self.addDockWidget(Qt.BottomDockWidgetArea, self.dock)  # Default position
            self.terminal.log("Set default editor_font_size to 12 and last_folder to home due to invalid settings", "INFO")
            self.apply_screen_size_and_position()
            self.apply_text_settings()
            self.apply_theme()
            self.apply_terminal_settings()
            ...
            self.settings["editor_font_size"] = 12
            self.addDockWidget(Qt.BottomDockWidgetArea, self.dock)  # Default position
            self.terminal.log("Set default editor_font_size to 12 and last_folder to home due to error", "INFO")
            self.apply_screen_size_and_position()
            self.apply_text_settings()
            self.apply_theme()
            self.apply_terminal_settings()
    ...
    def set_default_geometry(self):
        """Set default window geometry if settings are unavailable."""
