...
from PyQt5.QtWidgets import (
    ...
    QRadioButton, QButtonGroup, QSpinBox, QTabBar, QComboBox
)
...
class SettingsDialog(QDialog):
    def __init__(self, parent, color_scheme, show_line_numbers, tasks_file, gcbasic_path, show_info_logs, show_error_logs, window_width, window_height, menu_font_size, recent_files_font_size, tab_font_size, word_wrap, show_save_confirmation, tasks, f5_f6_f7_assignments):
        super().__init__(parent)
        self.setWindowTitle("IDE Settings")
        self.layout = QVBoxLayout()
        self.tasks = tasks
        self.f5_f6_f7_assignments = f5_f6_f7_assignments.copy()
        ...
        # New Section: Shortcut Assignments
        self.shortcut_label = QLabel("Shortcut Assignments:")
        self.layout.addWidget(self.shortcut_label)
        self.f5_combo = QComboBox(self)
        self.f6_combo = QComboBox(self)
        self.f7_combo = QComboBox(self)
        self.shift_f7_combo = QComboBox(self)
        self.task_combos = {
            "F5": self.f5_combo,
            "F6": self.f6_combo,
            "F7": self.f7_combo,
            "Shift+F7": self.shift_f7_combo
        }
        task_labels = ["None"] + [task["label"] for task in tasks]
        default_tasks = {
            "F5": "Make Hex and Flash",
            "F6": "Make Hex",
            "F7": "Make ASM",
            "Shift+F7": "Open ASM"
        }
        for key, combo in self.task_combos.items():
            ...
            current_task = f5_f6_f7_assignments.get(key, default_tasks.get(key))
            if current_task in task_labels:
                combo.setCurrentText(current_task)
            else:
                combo.setCurrentText("None")
            combo.currentTextChanged.connect(lambda text, k=key: self.update_assignment(k, text))
        self.layout.addWidget(QLabel("F5 Task:"))
        self.layout.addWidget(self.f5_combo)
        self.layout.addWidget(QLabel("F6 Task:"))
        self.layout.addWidget(self.f6_combo)
        self.layout.addWidget(QLabel("F7 Task:"))
        self.layout.addWidget(self.f7_combo)
        self.layout.addWidget(QLabel("Shift+F7 Task:"))
        self.layout.addWidget(self.shift_f7_combo)
    ...
    def update_assignment(self, key, text):
        self.f5_f6_f7_assignments[key] = text if text != "None" else None
    ...
    def apply_settings(self):
        ...
        self.parent().f5_f6_f7_assignments.update(self.f5_f6_f7_assignments)
        self.parent().apply_menu_font()
        self.parent().apply_tab_font()
        self.parent().update_ide_operations_menu()
...
class CustomTabBar(QTabBar):
    ...
    def show_context_menu(self, pos):
        ...
        copy_path_action.triggered.connect(lambda: self.copy_tab_path(tab_index))
        menu.addAction(copy_path_action)
        menu.exec_(self.mapToGlobal(pos))

    def copy_tab_path(self, tab_index):
        widget = self.ide.tab_widget.widget(tab_index)
        file_path = next((k for k, v in self.ide.editors.items() if v == widget), None)
        if file_path:
            QApplication.clipboard().setText(file_path)
            if self.ide.show_info_logs:
                self.ide.log_to_terminal(f"Copied file path to clipboard: {file_path}", "Info")
        else:
            if self.ide.show_info_logs:
                self.ide.log_to_terminal("No file path available for this tab (Untitled).", "Info")

class GCBASICEssentialIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GCBASIC CHEP IDE")
        self.f5_f6_f7_assignments = {
            "F5": "Make Hex and Flash",
            "F6": "Make Hex",
            "F7": "Make ASM"
        }
    ...
    def update_ide_operations_menu(self):
        ...
        # Add F5, F6, F7, Shift+F7 actions
        for key in ["F5", "F6", "F7", "Shift+F7"]:
            task_label = self.f5_f6_f7_assignments.get(key, None)
            label = f"Run {key} ({task_label or 'Unassigned'})"
            ...
            if self.show_info_logs:
                self.log_to_terminal(f"Added IDE operation action: {label}", "Info")

        used_keys = set(["F4", "F5", "F6", "F7", "Shift+F7"])
        for task in self.tasks:
            key = task.get("key")
            if key and key.startswith("F") and key[1:].isdigit() and 4 <= int(key[1:]) <= 12:
                if key in used_keys:
                    if self.show_error_logs:
    ...
    def execute_task(self, key):
        ...
        if editor and editor.document().isModified():
            ...
            if not self.save_if_modified(editor.file_path):
                ...
                return
        task_label = None
        if key in self.f5_f6_f7_assignments:
            task_label = self.f5_f6_f7_assignments[key]
            if task_label:
                if self.show_info_logs:
                    self.log_to_terminal(f"execute_task: Found task '{task_label}' for key {key}", "Info")
                self.run_task(task_label)
                return
            else:
                if self.show_info_logs:
                    self.log_to_terminal(f"No task assigned to {key}", "Info")
                return
        for task in self.tasks:
            if task.get("key") == key:
                ...
                self.run_task(task_label)
                if self.show_info_logs:
                    self.log_to_terminal(f"execute_task: Task '{task_label}' execution triggered.", "Info")
                return
        if self.show_error_logs:
    ...
    def load_tasks(self):
        ...
        try:
            ...
            if self.show_info_logs:
                self.log_to_terminal(f"Loaded {len(self.tasks)} tasks: {[task['label'] for task in self.tasks]}", "Info")
            key_counts = {}
            for task in self.tasks:
            ...
            # Assign tasks to F5, F6, F7, Shift+F7
            default_tasks = {
                "F5": "Make Hex and Flash [F5]",
                "F6": "Make HEX [F6]",
                "F7": "Make ASM [F7]",
                "Shift+F7": "Open ASM [Shift+F7]"
            }
            for key, default_label in default_tasks.items():
                found = False
                for task in self.tasks:
                    task_label = task.get("label", "").strip()
                    if task_label.lower() == default_label.lower():
                        self.f5_f6_f7_assignments[key] = task_label
                        found = True
                        if self.show_info_logs:
                            self.log_to_terminal(f"Assigned {key} to task '{task_label}'", "Info")
                        break
                if not found:
                    self.f5_f6_f7_assignments[key] = default_label
                    if self.show_error_logs:
                        self.log_to_terminal(f"Task '{default_label}' not found in tasks.json, using default for {key}", "Warning")
            self.update_ide_operations_menu()
    ...
    def load_settings(self):
        ...
        if os.path.exists(settings_file):
            try:
                ...
                self.f5_f6_f7_assignments = settings.get("f5_f6_f7_assignments", {
                    "F5": "Make Hex and Flash [F5]",
                    "F6": "Make HEX [F6]",
                    "F7": "Make ASM [F7]",
                    "Shift+F7": "Open ASM [Shift+F7]"
    ...
    def save_settings(self):
        settings = {
            ...
            "f5_f6_f7_assignments": self.f5_f6_f7_assignments,
