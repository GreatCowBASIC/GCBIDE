...
class SyntaxHighlighter:
            ...
            with open(language_file, "r", encoding="utf-8") as f:
                ...
                for rule in config.get("patterns", []):
                    try:
                        ...
                        self.highlighting_rules.append((re.compile(pattern, flags), format))
                        if SHOW_HL_INFO:
    ...
    def _apply_highlighting(self):
        ...
        for block, block_in_comment in blocks_to_highlight:
            ...
            format_ranges = []
            if SHOW_HL_INFO:
                self.ide.terminal.log(f"HL: Processing block {block.blockNumber()}, text length: {block_length}, in_block_comment: {block_in_comment}", "INFO")
                ...
                for pattern, format in self.highlighting_rules[1:]:  # Skip block comment rule
                    ...
                    for match in pattern.finditer(text):
                        ...
                        if not overlaps and end <= block_length:
                            ...
                            if end > block_length:
                                self.ide.terminal.log(f"HL: Invalid range ({start}, {end}) in block {block.blockNumber()}", "ERROR")
                        match_count += 1
