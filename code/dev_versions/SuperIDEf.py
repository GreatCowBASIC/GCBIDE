...
SHOW_HL_INFO = True  # Enabled for debugging
...
class SyntaxHighlighter:
    ...
    def schedule_highlighting(self):
        ...
        if not self.highlight_pending:
            ...
            self.highlight_timer.start(100)  # 100ms debounce

    def _apply_highlighting(self):
        """Apply highlighting to visible text in the active tab."""
        ...
        current_visible_range = (first_visible_block.blockNumber(), last_visible_block.blockNumber() if last_visible_block.isValid() else doc.blockCount() - 1)

        # Skip if the visible range hasn't changed significantly
        if self.last_visible_range == current_visible_range and not self.text_edit.document().isModified():
            self.highlight_pending = False
            return

        self.last_visible_range = current_visible_range

        # Determine which blocks need highlighting
        blocks_to_highlight = []
        block = first_visible_block

        # Walk backward from the first visible block to determine initial in_block_comment state
        in_block_comment = False
        temp_block = first_visible_block
        while temp_block.isValid():
            block_text = temp_block.text()
            # Check for comment end first (reverse order)
            end_match = self.block_comment_end.search(block_text)
            if end_match:
                # Found a comment end; look for a start before it
                start_block = temp_block
                while start_block.isValid():
                    start_text = start_block.text()
                    start_match = self.block_comment_start.search(start_text)
                    if start_match:
                        # If start is found before end, we're not in a comment unless start is after end
                        start_pos = start_match.start()
                        end_pos = end_match.end() if start_block == temp_block else len(start_text)
                        if start_block == temp_block and start_pos > end_pos:
                            in_block_comment = True
                        break
                    start_block = start_block.previous()
                if not start_block.isValid():
                    # No start found; assume we're in a comment
                    in_block_comment = True
                break
            else:
                # Check for comment start
                start_match = self.block_comment_start.search(block_text)
                if start_match:
                    # Found a start; check if its closed in the same block
                    end_match = self.block_comment_end.search(block_text, start_match.start())
                    if not end_match or end_match.end() > len(block_text):
                        in_block_comment = True
                    break
            temp_block = temp_block.previous()

        while block.isValid() and (not last_visible_block.isValid() or block.blockNumber() <= last_visible_block.blockNumber()):
            block_data = block.userData()
            block_text = block.text()
            if block.blockNumber() not in self.highlighted_blocks or (block_data is None or block_data.get_text() != block_text):
                blocks_to_highlight.append((block, in_block_comment))
            # Update in_block_comment for the next block
            if in_block_comment:
                end_match = self.block_comment_end.search(block_text)
                if end_match and end_match.end() <= len(block_text):
                    in_block_comment = False
                else:
                    in_block_comment = True
            else:
        ...
        for block, block_in_comment in blocks_to_highlight:
            text = block.text()
            block_length = len(text)
            format_ranges = []
            if SHOW_HL_INFO:
                self.ide.terminal.log(f"HL: Processing block {block.blockNumber()}, text length: {block_length}, in_block_comment: {block_in_comment}, text: '{text}'", "INFO")
            ...
            if block_in_comment:
                ...
                if end_match:
                    ...
                    if end_pos <= block_length:
                        # Highlight from start of block to end delimiter
                        format_ranges.append((0, end_pos, self.highlighting_rules[0][1]))
                        if SHOW_HL_INFO:
                            self.ide.terminal.log(f"HL: Block comment end matched at pos {end_pos} in block {block.blockNumber()}", "INFO")
                    else:
                        self.ide.terminal.log(f"HL: Invalid end_pos {end_pos} in block {block.blockNumber()}", "ERROR")
                else:
                    # Highlight entire block as part of block comment
                    format_ranges.append((0, block_length, self.highlighting_rules[0][1]))
                    if SHOW_HL_INFO:
                        self.ide.terminal.log(f"HL: Entire block {block.blockNumber()} highlighted as block comment", "INFO")
            else:
                pos = 0
                while pos < block_length:
                    start_match = self.block_comment_start.search(text, pos)
                    if not start_match:
                        break
                    start_pos = start_match.start()
                    ...
                    if end_match:
                        end_pos = end_match.end()
                        if end_pos <= block_length:
                            # Highlight entire block comment including delimiters and text
                            format_ranges.append((start_pos, end_pos, self.highlighting_rules[0][1]))
                            pos = end_pos
                            if SHOW_HL_INFO:
                                self.ide.terminal.log(f"HL: Single-line block comment matched at ({start_pos}, {end_pos}) in block {block.blockNumber()}", "INFO")
                        else:
                            self.ide.terminal.log(f"HL: Invalid end_pos {end_pos} in block {block.blockNumber()}", "ERROR")
                            break
                    else:
                        # Start of unclosed block comment
                        format_ranges.append((start_pos, block_length, self.highlighting_rules[0][1]))
                        pos = block_length
                        if SHOW_HL_INFO:
                            self.ide.terminal.log(f"HL: Unclosed block comment matched at ({start_pos}, {block_length}) in block {block.blockNumber()}", "INFO")
                        break

            # Apply other highlighting rules only outside block comments
            if not block_in_comment:
                max_matches = 1000
                for pattern, format in self.highlighting_rules[1:]:  # Skip block comment rule
                    match_count = 0
                    for match in pattern.finditer(text):
                        ...
                        # Only apply if not overlapping with block comment
                        overlaps = False
                        ...
                        if not overlaps and end <= block_length:
                            ...
                            if SHOW_HL_INFO:
                                self.ide.terminal.log(f"HL: Matched pattern '{pattern.pattern}' at ({start}, {end}) in block {block.blockNumber()}, color: {format.foreground().color().name()}", "INFO")
                        else:
                            if end > block_length:
                                self.ide.terminal.log(f"HL: Invalid range ({start}, {end}) in block {block.blockNumber()} for pattern {pattern.pattern}", "ERROR")
                        match_count += 1

            if len(format_ranges) > max_total_ranges:
                self.ide.terminal.log(f"HL: Exceeded max total ranges ({max_total_ranges}) in block {block.blockNumber()}", "ERROR")
                self.highlight_pending = False
                return

            if SHOW_HL_INFO:
                self.ide.terminal.log(f"HL: Applying {len(format_ranges)} format ranges in block {block.blockNumber()}", "INFO")
                for start, end, format in format_ranges:
                    self.ide.terminal.log(f"HL: Format range ({start}, {end}), color: {format.foreground().color().name()}", "INFO")

            # Apply formats to the current block
            cursor = QTextCursor(block)
            cursor.setPosition(block.position())
            pos = 0
            current_format = QTextCharFormat()

            format_ranges.sort(key=lambda x: (x[0], -x[1]))
            for start, end, format in format_ranges:
                if end > block_length:
                    self.ide.terminal.log(f"HL: Skipping invalid range ({start}, {end}) in block {block.blockNumber()}", "ERROR")
                    continue
                if pos < start:
                    cursor.setPosition(block.position() + start)
                    cursor.setPosition(block.position() + start, QTextCursor.KeepAnchor)
            ...
            # Mark block as highlighted and store its text and comment state
            self.highlighted_blocks.add(block.blockNumber())
...
class CustomTextEdit(QTextEdit):
    ...
    def on_text_changed(self):
        """Schedule highlighting and clear cache for modified blocks."""
        if not self._is_highlighting:
            self._is_highlighting = True
            try:
                # Clear highlighted blocks cache on text change
                self.highlighter.highlighted_blocks.clear()
                self.highlighter.schedule_highlighting()
            except Exception as e:
...
class IDE(QMainWindow):
    ...
    def new_file(self):
        ...
        self.terminal.log(f"Created new file with editor_font_size: {self.settings['editor_font_size']}", "INFO")
        self.background_widget.update()
    ...
    def open_file_by_path(self, file_path):
        ...
        self.apply_text_settings(text_edit)
        text_edit.highlighter.schedule_highlighting()
        self.terminal.log(f"Opened file {file_path} with editor_font_size: {self.settings['editor_font_size']}", "INFO")
        if normalized_path in [self.normalize_path(p) for p in self.recent_files]:
            self.recent_files = [p for p in self.recent_files if self.normalize_path(p) != normalized_path]
        self.recent_files.insert(0, file_path)
        if len(self.recent_files) > 10:
    ...
    def apply_text_settings(self, text_edit=None):
        ...
        if text_edit is None:
            ...
            text_edit.line_number_area.update()
            text_edit.highlighter.schedule_highlighting()
            self.terminal.log(f"Applied editor_font_size {self.settings['editor_font_size']} to text edit", "INFO")
    ...
    def load_settings(self):
        ...
        try:
            with open(settings_path, "r", encoding="utf-8") as f:
                ...
                self.apply_theme()
                self.apply_terminal_settings()
                self.terminal.log(f"Loaded editor_font_size: {self.settings['editor_font_size']}", "INFO")
