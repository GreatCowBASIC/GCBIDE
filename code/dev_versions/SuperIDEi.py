#got broken - but, this has many working routine to handle the TASKS jdson
import sys
import os
import os.path
import json
...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QTabWidget, QTextEdit, QVBoxLayout, QWidget,
                             ...
                             QInputDialog, QMenu, QFrame, QDialog, QDialogButtonBox, QTextBrowser, QComboBox, QShortcut)
from PyQt5.QtPrintSupport import QPrintDialog, QPrinter
from PyQt5.QtGui import (QTextOption, QTextDocument, QFont, QPainter, QFontMetrics, QTextCursor, QIcon,
                         QTextCharFormat, QColor, QImage, QKeySequence)
...
class IDE(QMainWindow):
    def __init__(self, filename=None):
        ...
        self.settings = {
            ...
            "tasks_file": "C:\\GCstudio\\vscode\\data\\user-data\\User\\tasks.json"
        }
        self.recent_files = []
        self.file_cache = {}
        self.history = {}
    ...
    def init_ui(self):
        ...
        self.operations_menu = menubar.addMenu("&IDE Operations")
        ...
        # Add F4 shortcut to trigger task menu population
        self.operations_shortcut = QShortcut(QKeySequence("F4"), self)
        self.operations_shortcut.activated.connect(self.execute_tasks)
        ...
        tasks_menu = settings_menu.addMenu("&Tasks")
        ...
        tasks_file_action = QAction("Tasks &File", self)
        tasks_file_action.triggered.connect(self.set_tasks_file)
        tasks_menu.addAction(tasks_file_action)
    ...
    def execute_tasks(self):
        """Populate the IDE Operations menu with tasks from tasks.json."""
        self.terminal.log("Populating IDE Operations menu with tasks", "INFO")
        current_tab = self.tabs.currentWidget()
        if not current_tab or not hasattr(current_tab, "file_path"):
            self.terminal.log("No file open to execute tasks", "ERROR")
            return

        # Clear existing actions from the operations menu
        self.operations_menu.clear()

        # Load tasks from tasks.json
        tasks = self.load_tasks()
        if not tasks:
            self.terminal.log("No tasks available to execute", "ERROR")
            no_tasks_action = QAction("No Tasks Available", self)
            no_tasks_action.setEnabled(False)
            self.operations_menu.addAction(no_tasks_action)
        else:
            # Create a QAction for each task
            for task in tasks:
                task_label = task["label"]
                task_action = QAction(task_label, self)
                task_action.triggered.connect(lambda checked, t=task: self.run_task(t, current_tab.file_path))
                self.operations_menu.addAction(task_action)

        # Open the operations menu at the IDE Operations menu position
        action_pos = self.menuBar().actionGeometry(self.operations_menu.menuAction())
        menu_pos = self.menuBar().mapToGlobal(action_pos.bottomLeft())
        self.operations_menu.popup(menu_pos)
    ...
    def set_tasks_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Tasks File", "", "JSON Files (*.json)")
        if file_path:
            self.settings["tasks_file"] = file_path
            self.save_settings()
            self.terminal.log(f"Set tasks file to {file_path}", "INFO")
    ...
    def get_tasks_file_path(self):
        """Determine the tasks file path based on settings or fallback locations."""
        # Primary location from settings
        tasks_file = self.settings.get("tasks_file", "C:\\GCstudio\\vscode\\data\\user-data\\User\\tasks.json")
        if os.path.exists(tasks_file):
            return tasks_file

        # Secondary location: local folder
        local_tasks = os.path.join(os.path.dirname(__file__), "tasks.json")
        if os.path.exists(local_tasks):
            return local_tasks

        # Tertiary option: prompt user to select
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Tasks File", "", "JSON Files (*.json)")
        ...
        self.terminal.log("No valid tasks.json file found", "ERROR")
        return None

    def load_tasks(self):
        """Load tasks from the tasks.json file, supporting JSONC with comments by skipping lines starting with //."""
        tasks_file = self.get_tasks_file_path()
        self.terminal.log(f"Attempting to load tasks from: {tasks_file}", "INFO")
        if not tasks_file:
            self.terminal.log("No tasks file found, returning empty task list", "INFO")
            return []

        try:
            self.terminal.log(f"Opening tasks file: {tasks_file}", "INFO")
            with open(tasks_file, "r", encoding="utf-8") as f:
                # Read lines and skip comments starting with //
                lines = f.readlines()
                # Debug: Log raw content for lines 518..523
                self.terminal.log(f"Raw content lines 518..523:", "INFO")
                for i in range(518, min(524, len(lines))):
                    line_content = lines[i - 1].rstrip('\n').replace('\r', '\\r').replace('\t', '\\t')
                    self.terminal.log(f"Line {i}: {line_content}", "INFO")
                ...
                self.terminal.log(f"Raw file content after reading lines (first 1000 chars):\n{content[:1000]}...", "INFO")
                # Remove block comments (/* */)
                content = re.sub(r'/\*.*?\*/', '', content, flags=re.DOTALL)
                # Debug: Log content after stripping comments for lines 518..523
                stripped_lines = content.splitlines()
                self.terminal.log(f"Content after stripping comments, lines 518..523:", "INFO")
                for i in range(518, min(524, len(stripped_lines))):
                    line_content = stripped_lines[i - 1].rstrip('\n').replace('\r', '\\r').replace('\t', '\\t')
                    self.terminal.log(f"Line {i}: {line_content}", "INFO")
                # Debug: Log the "View GC BASIC Demos" task if present
                view_demos_index = content.find('"View GC BASIC Demos"')
                if view_demos_index != -1:
                    task_start = content.rfind('{', 0, view_demos_index)
                    task_end = content.find('}', view_demos_index) + 1
                    task_content = content[task_start:task_end].replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t')
                    self.terminal.log(f"View GC BASIC Demos task after stripping comments:\n{task_content}", "INFO")
                self.terminal.log(f"Content after stripping comments (first 1000 chars):\n{content[:1000]}...", "INFO")
                # Sanitize control characters (ASCII 0-31, except \n, \r, \t)
                content = ''.join(c if ord(c) >= 32 or c in '\n\r\t' else '' for c in content)
                # Debug: Log content after sanitizing control characters for lines 518..523
                sanitized_lines = content.splitlines()
                self.terminal.log(f"Content after sanitizing control characters, lines 518..523:", "INFO")
                for i in range(518, min(524, len(sanitized_lines))):
                    line_content = sanitized_lines[i - 1].rstrip('\n').replace('\r', '\\r').replace('\t', '\\t')
                    self.terminal.log(f"Line {i}: {line_content}", "INFO")
                # Debug: Log the "View GC BASIC Demos" task after sanitization
                view_demos_index = content.find('"View GC BASIC Demos"')
                if view_demos_index != -1:
                    task_start = content.rfind('{', 0, view_demos_index)
                    task_end = content.find('}', view_demos_index) + 1
                    task_content = content[task_start:task_end].replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t')
                    self.terminal.log(f"View GC BASIC Demos task after sanitizing control characters:\n{task_content}", "INFO")
                self.terminal.log(f"Content after sanitizing control characters (first 1000 chars):\n{content[:1000]}...", "INFO")
                # Parse the sanitized content
                data = json.loads(content)
                self.terminal.log(f"Raw JSON data loaded: {json.dumps(data, indent=2)[:1000]}...", "INFO")
                tasks = data.get("tasks", []) if isinstance(data, dict) else data
                self.terminal.log(f"Extracted tasks: {json.dumps(tasks, indent=2)[:1000]}...", "INFO")
                filtered_tasks = [task for task in tasks if "label" in task and "command" in task]
                self.terminal.log(f"Filtered tasks (with label and command): {json.dumps(filtered_tasks, indent=2)[:1000]}...", "INFO")
                return filtered_tasks
        except json.JSONDecodeError as e:
            self.terminal.log(f"Invalid JSON in {tasks_file}: {str(e)}", "ERROR")
            # Log the problematic line and surrounding context
            lines = content.splitlines()
            line_num = e.lineno
            if 0 < line_num <= len(lines):
                problematic_line = lines[line_num - 1].strip()
                self.terminal.log(f"Problematic line {line_num}: {problematic_line}", "INFO")
                # Log 20 characters before and after the error position
                pos = e.pos
                start = max(0, pos - 20)
                end = min(len(content), pos + 20)
                snippet = content[start:end].replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t')
                self.terminal.log(f"Content around error (char {pos}): {snippet}", "INFO")
                # Debug: Log surrounding lines
                self.terminal.log(f"Content lines {max(1, line_num-2)}..{min(len(lines), line_num+2)}:", "INFO")
                for i in range(max(1, line_num-2), min(len(lines)+1, line_num+3)):
                    line_content = lines[i - 1].rstrip('\n').replace('\r', '\\r').replace('\t', '\\t')
                    self.terminal.log(f"Line {i}: {line_content}", "INFO")
            else:
                self.terminal.log(f"Error at line {line_num}, but file has {len(lines)} lines", "INFO")
            self.terminal.log("Returning empty task list due to JSON error", "INFO")
            return []
        except Exception as e:
            self.terminal.log(f"Error loading {tasks_file}: {str(e)}", "ERROR")
            self.terminal.log("Returning empty task list due to error", "INFO")
            return []
...
def execute_tasks(self):
        ...
        # Open the operations menu at the menu bar position
        menu_pos = self.menuBar().mapToGlobal(QPoint(0, self.menuBar().height()))
        self.operations_menu.exec_(menu_pos)

def run_task(self, task, file_path):
        """Execute the selected task using the operating system call."""
        try:
            command = task.get("command", "")
            args = task.get("args", [])
            options = task.get("options", {})
            cwd = options.get("cwd", os.path.dirname(file_path))

            # Log raw cwd
            self.terminal.log(f"Raw working directory: {cwd}", "INFO")

            # Convert VS Code ${env:VARIABLE} syntax to %VARIABLE%
            cwd = re.sub(r'\${env:([^}]+)}', r'%\1%', cwd)
            self.terminal.log(f"Converted working directory (VS Code syntax): {cwd}", "INFO")

            # Resolve environment variables and normalize
            cwd = os.path.expandvars(cwd).rstrip(os.sep)
            self.terminal.log(f"Resolved working directory: {cwd}", "INFO")

            # Validate cwd
            if not cwd or not os.path.exists(cwd):
                self.terminal.log(f"Invalid working directory: {cwd}", "ERROR")
                # Fallback to file directory
                cwd = os.path.dirname(file_path)
                self.terminal.log(f"Falling back to file directory: {cwd}", "INFO")
                if not os.path.exists(cwd):
                    raise ValueError(f"Fallback working directory '{cwd}' does not exist")

            # Check if environment variable was resolved
            if '%' in cwd or '$' in cwd:
                self.terminal.log(f"Environment variable not resolved in cwd: {cwd}", "ERROR")
                raise ValueError(f"Environment variable not resolved in working directory '{cwd}'")

            # Normalize file paths in command and args
            command = re.sub(r'\${env:([^}]+)}', r'%\1%', command)
            command = os.path.normpath(os.path.expandvars(command))
            self.terminal.log(f"Resolved command: {command}", "INFO")

            normalized_args = []
            for arg in args:
                # Replace placeholders like ${file} and normalize paths
                arg = arg.replace("${file}", file_path)
                arg = arg.replace("${fileDirname}", os.path.dirname(file_path))
                arg = re.sub(r'\${env:([^}]+)}', r'%\1%', arg)
                arg = os.path.expandvars(arg)
                normalized_args.append(arg)

            # Combine command and arguments
            full_command = [command] + normalized_args
            self.terminal.log(f"Executing task '{task['label']}': {' '.join(full_command)}", "INFO")
            ...
            process = subprocess.Popen(
                ...
                text=True,
                shell=True if task.get("type") == "shell" else False
            )

            # Capture output
            stdout, stderr = process.communicate()
            if stdout:
                self.terminal.log(f"Task output: {stdout}", "INFO")
            if stderr:
                self.terminal.log(f"Task error: {stderr}", "ERROR")

            if process.returncode == 0:
                self.terminal.log(f"Task '{task['label']}' completed successfully", "INFO")
            else:
                self.terminal.log(f"Task '{task['label']}' failed with exit code {process.returncode}", "ERROR")

        except ValueError as e:
            self.terminal.log(f"Error executing task '{task.get('label', 'unknown')}': {str(e)}", "ERROR")
        except Exception as e:
            self.terminal.log(f"Error executing task '{task.get('label', 'unknown')}': {str(e)}", "ERROR")
...
def run_task(self, task, file_path):
        ...
        try:
            ...
            for arg in args:
                # Log raw argument
                self.terminal.log(f"Raw argument: {arg}", "INFO")
                # Replace placeholders like ${file}
                arg = arg.replace("${file}", file_path)
                arg = arg.replace("${fileDirname}", os.path.dirname(file_path))
                # Convert VS Code env syntax
                arg = re.sub(r'\${env:([^}]+)}', r'%\1%', arg)
                arg = os.path.expandvars(arg)
                # Replace single quotes with double quotes for paths
                if arg.startswith("'") and arg.endswith("'") and (':' in arg or '\\' in arg or '/' in arg):
                    arg = f'"{arg[1:-1]}"'
                elif (':' in arg or '\\' in arg or '/' in arg) and not arg.startswith('"'):
                    arg = f'"{arg}"'
                self.terminal.log(f"Processed argument: {arg}", "INFO")
                normalized_args.append(arg)
