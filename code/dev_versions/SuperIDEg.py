# issues
# - file open and save issues odd messages
# - no toolbar support
# - highlighting works
# - the editor goes multiline sometimes
...
class SyntaxHighlighter:
    ...
    def _apply_highlighting(self):
        """Apply highlighting to changed or visible text, preserving undo stack."""
        ...
        doc = self.text_edit.document()
        #ERV self.ide.terminal.log(f"Before highlighting - isUndoAvailable: {doc.isUndoAvailable()}, isModified: {doc.isModified()}", "INFO")
        ...
        try:
            ...
            #ERV self.ide.terminal.log(f"After highlighting - isUndoAvailable: {doc.isUndoAvailable()}, isModified: {doc.isModified()}", "INFO")
            self.highlight_pending = False
...
def _apply_highlighting(self):
    ...
    doc = self.text_edit.document()
    self.ide.terminal.log(f"Before highlighting - isUndoAvailable: {doc.isUndoAvailable()}, isModified: {doc.isModified()}", "INFO")
...
class CustomTextEdit(QTextEdit):
    def __init__(self, ide):
        ...
        #ERV self.ide.terminal.log(f"CustomTextEdit initialized - undoRedoEnabled: {self.isUndoRedoEnabled()}, document undoRedoEnabled: {self.document().isUndoRedoEnabled()}", "INFO")
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Z and event.modifiers() == Qt.ControlModifier:
            #ERV self.ide.terminal.log("Ctrl+Z intercepted in CustomTextEdit, forwarding to IDE.undo", "INFO")
            self.ide.undo()
            return
        if event.key() == Qt.Key_Tab:
            ...
            self.ide.dedent()
            return
        #ERV self.ide.terminal.log(f"Key pressed - key: {event.key()}, text: '{event.text()}', undoAvailable: {self.document().isUndoAvailable()}", "INFO")
    ...
    def on_text_changed(self):
        ...
        if not self._is_highlighting:
            ...
            try:
                #ERV self.ide.terminal.log(f"Text changed - isUndoAvailable: {self.document().isUndoAvailable()}, isModified: {self.document().isModified()}", "INFO")
...
class IDE(QMainWindow):
    ...
    def new_file(self):
        ...
        #ERV self.terminal.log(f"Created new file - undoRedoEnabled: {text_edit.isUndoRedoEnabled()}, isUndoAvailable: {text_edit.document().isUndoAvailable()}, isModified: {text_edit.document().isModified()}", "INFO")
        self.background_widget.update()
    ...
    def open_file_by_path(self, file_path):
        ...
        self.apply_text_settings(text_edit)
        text_edit.highlighter.schedule_highlighting()
        #ERV self.terminal.log(f"Opened file {file_path} - undoRedoEnabled: {text_edit.isUndoRedoEnabled()}, isUndoAvailable: {text_edit.document().isUndoAvailable()}, isModified: {text_edit.document().isModified()}", "INFO")
        if normalized_path in [self.normalize_path(p) for p in self.recent_files]:
            self.recent_files = [p for p in self.recent_files if self.normalize_path(p) != normalized_path]
        self.recent_files.insert(0, file_path)
        if len(self.recent_files) > 10:
    ...
    def undo(self):
        ...
        if current_tab:
            ...
            #ERV self.terminal.log(f"Undo requested - isUndoAvailable: {doc.isUndoAvailable()}, modified: {doc.isModified()}", "INFO")
