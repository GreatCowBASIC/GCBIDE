...
class IDE(QMainWindow):
    def __init__(self, filename=None):
        ...
        if os.path.exists(icon_path):
            ...
            # INFO commented out reduce information overload
            # self.terminal.log(f"Application icon loaded: {icon_path}", "INFO")
            pass
        else:
    ...
    def normalize_path(self, path):
        """Normalize a file path to use consistent separators and resolve to absolute."""
    ...
    def show_recent_files(self):
        menu = QMenu()
        menu.setFont(QFont("Arial", self.settings["font_size"]))  # Set font to match IDE settings
        seen = set()
        for index, file in enumerate(self.recent_files[:10], 1):
            ...
            if normalized_file not in seen and os.path.exists(file):
                ...
                action.setFont(QFont("Arial", self.settings["font_size"]))  # Ensure action font matches
        ...
        if not seen:
            ...
            action.setFont(QFont("Arial", self.settings["font_size"]))  # Apply font to "No recent files"
    ...
    def load_settings(self):
        ...
        try:
            with open(settings_path, "r", encoding="utf-8") as f:
                ...
                # Normalize recent files on load to eliminate duplicates
