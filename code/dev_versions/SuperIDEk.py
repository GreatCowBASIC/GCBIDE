# issues
# - Tasks work
# - Fixed new setting file.  Handles correctly now.
...
class IDE(QMainWindow):
    ...
    def init_ui(self):
        ...
        run_task_action = QAction("&Run Task", self)
        run_task_action.setShortcut("F4")
    ...
    def run_task(self):
        """Display tasks from tasks.json and execute the selected task."""
        ...
        layout = QVBoxLayout()
        combo = QComboBox()
        task_labels = [task.get("label", "Unnamed Task") for task in tasks]
    ...
    def execute_task(self, task, file_path):
        ...
        try:
            ...
            self.terminal.log(f"Executing task: {' '.join(full_command)} in {cwd}", "INFO")
            if "make" in task.get("label", "").lower():
                        ...
                        try:
                            with open(output_file, 'r', encoding='utf-8') as f:
                                ...
                                if not lines:
                                    ...
                                    self.terminal.log(f"Read {len(lines)} lines from {output_file}: {lines[0][:50]!r}", "INFO")
                                for line in lines:
                                    line = line.rstrip('\n')
