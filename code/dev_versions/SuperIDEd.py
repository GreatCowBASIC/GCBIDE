...
class SyntaxHighlighter:
            ...
            with open(language_file, "r", encoding="utf-8") as f:
                ...
                for rule in config.get("patterns", []):
                    try:
                        ...
                        if SHOW_HL_INFO:
                            self.ide.terminal.log(f"HL: Loaded rule - Pattern: {pattern}, Color: {rule['color']}", "INFO")
                    except re.error as e:
    ...
    def highlight_visible_text(self):
        ...
        while block.isValid() and (not last_visible_block.isValid() or block.blockNumber() <= last_visible_block.blockNumber()):
            ...
            format_ranges = []
            if SHOW_HL_INFO:
                self.ide.terminal.log(f"HL: Processing block {block.blockNumber()}, text length: {block_length}", "INFO")
            ...
            if in_block_comment:
                ...
                start_pos = 0
                while True:
                    start_match = self.block_comment_start.search(text[start_pos:])
                    if not start_match:
                        break
...
class CustomTextEdit(QTextEdit):
    ...
    def keyPressEvent(self, event):
        ...
        if not self._is_highlighting:
            self.highlighter.highlight_visible_text()
...
class IDE(QMainWindow):
    ...
    def new_file(self):
        ...
        text_edit.highlighter.highlight_visible_text()
    ...
    def undo(self):
        ...
        if current_tab:
            current_tab.undo()
            current_tab.highlighter.highlight_visible_text()
    ...
    def set_theme(self):
        ...
        if ok:
            ...
            self.save_settings()
            for i in range(self.tabs.count()):
                self.tabs.widget(i).highlighter.highlight_visible_text()
