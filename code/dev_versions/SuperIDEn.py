# issues
# - None (Tasks assigned to Function keys!)
...
class IDE(QMainWindow):
        ...
        if not tasks:
            ...
            for task in tasks:
                ...
                if shortcut:
                    ...
                    if SHOW_FN_INFO:
                        self.terminal.log(f"FN: Assigned shortcut '{shortcut}' to task '{label}'", "INFO")
        ...
        try:
            ...
            for task in tasks:
                ...
                if match:
                    ...
                    if SHOW_FN_INFO:
                        self.terminal.log(f"FN: Parsed shortcut '{shortcut}' for task '{label}'", "INFO")
                else:
                    task["shortcut"] = None
                    if SHOW_FN_INFO:
                        self.terminal.log(f"FN: No valid shortcut found in task label '{label}'", "INFO")
            return tasks
        except json.JSONDecodeError as e:
    ...
    def get_tasks_file_path(self):
        """Determine the tasks.json file path based on settings and fallbacks."""
        ...
        if os.path.exists(local_tasks):
            return local_tasks
        # Fallback: prompt user to select a file
    ...
    def run_task(self, task=None):
        ...
        if not current_tab or not hasattr(current_tab, "file_path"):
            self.terminal.log("No file open to run task", "ERROR")
            return
        if task:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                self.execute_task(selected_task, current_tab.file_path)

    def execute_task(self, task, file_path):
        """Execute the selected task using the operating system shell."""
        import time
        try:
            ...
            current_tab = self.tabs.currentWidget()
            # Save modified file if label contains "make" (case-insensitive)
            if "make" in task.get("label", "").lower():
                if current_tab and hasattr(current_tab, "file_path") and current_tab.document().isModified():
                    self.save_file()
                    self.terminal.log(f"Saved file {current_tab.file_path} before executing 'make' task", "INFO")
            # Normalize and quote paths for command and cwd
            env_vars = os.environ
            ...
            command = command.replace("${fileDirname}", os.path.dirname(file_path))
            # Check for ${execPath} placeholder
            if "${execPath}" in command:
                # Open file_path in a new tab
                if os.path.exists(file_path):
                    self.open_file_by_path(file_path)
                    self.terminal.log(f"Opened file in new tab: {file_path}", "INFO")
                else:
                    self.terminal.log(f"File not found for new tab: {file_path}", "ERROR")
                # Get current tab's filename and open .ASM version
                current_tab = self.tabs.currentWidget()
                ...
                return  # Exit task
            # Ensure command is quoted if it contains path separators
            ...
            if not os.path.exists(file_path):
                self.terminal.log(f"Input file not found: {file_path}", "ERROR")
                return
            if not os.path.exists(cwd):
                self.terminal.log(f"Working directory not found: {cwd}", "ERROR")
                return
            # Process arguments, transforming single-quoted placeholders to double quotes
            processed_args = []
            ...
            QApplication.processEvents()  # Ensure immediate display
            if "make" in task.get("label", "").lower():
                # Run the command and capture output to a temporary file for "make" tasks
                with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', delete=False, suffix='.txt') as temp_file:
                    process = subprocess.run(' '.join(full_command), cwd=cwd, stdout=temp_file, stderr=temp_file, shell=True)
                # Read and display %temp%\gcbasic.log
                ...
                for _ in range(5):
                    if os.path.exists(output_file):
                        try:
                            with open(output_file, 'r', encoding='utf-8') as f:
                                ...
                                if not lines:
                                    ...
                                    pass
                                    # self.terminal.log(f"Read {len(lines)} lines from {output_file}: {lines[0][:50]!r}\n", "INFO")
                                for line in lines:
                                    line = line.rstrip('\n')
                        ...
                        break
                # Check for errors.txt in the same folder as GCBASIC.EXE
                ...
                for _ in range(5):
                    if os.path.exists(errors_file):
                        try:
                            ...
                            time.sleep(1.0)
                    break  # No error if errors.txt doesn't exist
                ...
                # Launch non-"make" tasks non-modally without error reporting
                current_tab = self.tabs.currentWidget()
                if current_tab:
                ...
                if self.settings["show_info"]:
                    ...
                    self.terminal.addItem("")  # Blank line
                    if not self.terminal.user_scrolled or self.terminal.verticalScrollBar().value() == self.terminal.verticalScrollBar().maximum():
                        self.terminal.scrollToBottom()
                        self.terminal.user_scrolled = False
        except Exception as e:
    ...
    def set_tasks_file(self):
        """Set the tasks.json file location via file dialog."""
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Tasks File", "", "JSON Files (*.json)")
        if file_path:
            self.settings["tasks_file"] = file_path
            self.save_settings()
            self.terminal.log(f"Set tasks file to {file_path}", "INFO")
            # Repopulate tasks menu after changing tasks file
            self.populate_tasks_menu()
    ...
    def load_settings(self):
        ...
        try:
            ...
            try:
                self.recent_files = []  # Initialize recent_files
                self.settings["recent_files"] = self.recent_files
                ...
                # Log secondary exception for debugging (temporary)
                self.terminal.log("No IDE setting file found using default parameters", "INFO")
                # Set minimal defaults silently
                self.recent_files = []
                self.settings["recent_files"] = self.recent_files
                self.apply_screen_size_and_position()
                self.apply_text_settings()
