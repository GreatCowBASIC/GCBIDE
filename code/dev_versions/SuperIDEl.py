# issues
# - None (Tasks to Menu option resolved).
...
class CustomTextEdit(QTextEdit):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_F4:
            if SHOW_HL_INFO:
                self.ide.terminal.log("HL: F4 intercepted in CustomTextEdit, forwarding to IDE.open_tasks_menu", "INFO")
            self.ide.open_tasks_menu()
            return
...
class IDE(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Populate IDE Tasks menu dynamically
    ...
    #     self.ide_tasks_menu.addSeparator()
    #     select_task_action = QAction("Select Task...", self)
    #     select_task_action.triggered.connect(lambda: self.run_task())
    #     self.ide_tasks_menu.addAction(select_task_action)
