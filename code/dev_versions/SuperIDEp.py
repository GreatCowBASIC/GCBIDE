...
# works with new execute handler.  
# next to add is Management of the terminal window
...
class IDE(QMainWindow):
    ...
    #     # For make tasks, require an open tab
    #     if task and "make" in task.get("label", "").lower():
    #         current_tab = self.tabs.currentWidget()
    #         if not current_tab or not hasattr(current_tab, "file_path"):
    #             self.terminal.log("No file open to run make task", "ERROR")
    #             return
    #         self.execute_task(task, current_tab.file_path)
    #     else:
    #         # For non-make tasks, use empty file_path if no tab
    #         current_tab = self.tabs.currentWidget()
    ...
    #         local_file_path = file_path  # Use parameter, reassign for make tasks
    #         # Handle make tasks (require open tab)
    ...
    #                 self.save_file()
    #                 self.terminal.log(f"Saved file {local_file_path} before executing 'make' task", "INFO")
    ...
    #             if include_match:
    #                 extensions = [ext.strip() for ext in include_match.group(1).split(',')]
    #                 extensions = [ext.lstrip('*.') if ext.startswith('*.') else ext for ext in extensions]
    ...
    #                 if not deleted:
    #                     self.terminal.log(f"No files found to delete in {dirname} with extensions: {', '.join(extensions)}", "INFO")
    #                 return
